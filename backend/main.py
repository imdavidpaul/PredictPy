...
def _nb_cell(cell_type: str, source: str) -> dict:
    """Build a single nbformat 4 cell dict."""
    return {
        "cell_type": cell_type,
        "id": uuid.uuid4().hex[:8],
        "metadata": {},
...
def export_notebook(body: ExportNotebookRequest) -> Response:
    ...
    model_name = model_data["meta"].get("model_name", "Unknown") if model_data else "Unknown"
    ...
    cells.append(_nb_cell("markdown", "\n".join([
        ...
        f"| Features | {len(features)} selected |",
        f"",
        f"> Generated by [PredictPy](https://github.com). Re-run cells after loading your dataset.",
    ])))
    ...
    # Cell 3 — Configuration variables + CSV placeholder
    feature_repr = json.dumps(features, indent=4)
    cells.append(_nb_cell("code", "\n".join([
        f"target_column = {json.dumps(target)}",
        f"feature_columns = {feature_repr}",
        f"problem_type = {json.dumps(problem_type)}",
        "",
    ...
    # Cell 4 — Metrics DataFrame
    metrics = ev.get("metrics", {})
    if metrics:
        metric_lines = ["metrics_data = {"]
        for k, v in metrics.items():
            metric_lines.append(f"    {json.dumps(k)}: {json.dumps(v)},")
        metric_lines.append("}")
        metric_lines.append("pd.DataFrame([metrics_data]).T.rename(columns={0: 'Value'})")
    else:
        metric_lines = ["# No metrics available", "pd.DataFrame()"]
    cells.append(_nb_cell("code", "\n".join(metric_lines)))

    # Cell 5 — Primary chart (scatter or confusion matrix)
    if problem_type == "regression":
        actual_vs_pred = ev.get("actual_vs_predicted", [])
        actuals = [p["actual"] for p in actual_vs_pred]
        preds = [p["predicted"] for p in actual_vs_pred]
        cells.append(_nb_cell("code", "\n".join([
            f"actuals = {json.dumps(actuals[:500])}",
            f"predictions = {json.dumps(preds[:500])}",
            "",
            "fig, ax = plt.subplots(figsize=(7, 6))",
            "ax.scatter(actuals, predictions, alpha=0.5, color='#8b5cf6', s=20)",
            "lo, hi = min(actuals + predictions), max(actuals + predictions)",
        ...
        cm_matrix = cm.get("matrix", [])
        cm_labels = cm.get("labels", [])
        cells.append(_nb_cell("code", "\n".join([
            f"cm_matrix = {json.dumps(cm_matrix)}",
            f"cm_labels = {json.dumps(cm_labels)}",
            "",
            "fig, ax = plt.subplots(figsize=(7, 6))",
    ...
    # Cell 6 — Secondary chart (residual or ROC)
    if problem_type == "regression":
        cells.append(_nb_cell("code", "\n".join([
            "residuals = [a - p for a, p in zip(actuals, predictions)]",
            "",
            "fig, ax = plt.subplots(figsize=(7, 4))",
            "ax.scatter(predictions, residuals, alpha=0.5, color='#06b6d4', s=20)",
        ...
        fpr = roc.get("fpr", [])
        tpr = roc.get("tpr", [])
        auc_val = ev.get("metrics", {}).get("ROC AUC", None)
        auc_label = f", AUC={auc_val:.4f}" if isinstance(auc_val, (int, float)) else ""
        cells.append(_nb_cell("code", "\n".join([
            f"fpr = {json.dumps(fpr[:200])}",
            f"tpr = {json.dumps(tpr[:200])}",
            "",
            "fig, ax = plt.subplots(figsize=(6, 5))",
            f"ax.plot(fpr, tpr, color='#8b5cf6', lw=2, label=f'ROC curve{auc_label}')",
    ...
    # Cell 7 — Drift detection table
    drift_data = ev.get("drift", [])
    if drift_data:
        drift_lines = [f"drift_data = {json.dumps(drift_data)}"]
        drift_lines += [
            "drift_df = pd.DataFrame(drift_data)[['feature', 'test', 'p_value', 'drifted']]",
            "drift_df['status'] = drift_df['drifted'].map({True: 'Drifted', False: 'Stable'})",
            "drift_df.drop(columns='drifted').style.applymap(",
            "    lambda v: 'color: red' if v == 'Drifted' else 'color: green', subset=['status']",
            ")",
        ]
    else:
        drift_lines = ["# No drift data available", "pd.DataFrame()"]
    cells.append(_nb_cell("code", "\n".join(drift_lines)))

    # Cell 8 — Notes markdown
    cells.append(_nb_cell("markdown", "\n".join([
        "## Notes",
        "",
        "- Replace `your_dataset.csv` with the path to your evaluation dataset.",
        "- Load your `model.pkl` file using `pickle.load()` to regenerate predictions.",
        "- Metrics, charts, and drift data above are pre-embedded from the PredictPy session.",
        "",
        "---",
        "_Generated by [PredictPy](https://github.com)_",
    ])))

    # Strip None-valued keys to match nbformat spec
    clean_cells = []
    for c in cells:
        clean = {k: v for k, v in c.items() if v is not None}
        if "outputs" not in clean and clean.get("cell_type") == "code":
            clean["outputs"] = []
    ...
    return Response(
        ...
        headers={"Content-Disposition": 'attachment; filename="predictpy_evaluation.ipynb"'},
    )
